    ...
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                <div class="panel-heading">{{Auth::user()->school->name}} 的支付信息</div>

                <div class="panel-body">
                    贵校共{{Auth::user()->school->delegates->where('status', 'oVerified')->count()}}名未缴费代表，其中{{Auth::user()->school->delegates->where('status','oVerified')->where('accomodate', 1)->count()}}位代表住宿；{{Auth::user()->school->volunteers->where('status', 'oVerified')->count()}}名未缴费志愿者，其中{{Auth::user()->school->volunteers->where('status','oVerified')->where('accomodate', 1)->count()}}位志愿者住宿，总待缴费{{Auth::user()->school->toPayAmount()}}元。<br><br>
                    您共有两种缴费方式可以选择：<br>
                <section class="panel">
                ...
                <div class="panel-body">
                  <div class="tab-content">
                    <div class="tab-pane active" id="methods">您当前选择的缴费方式是：{{Auth::user()->school->payment_method == 'individual' ? '个人缴费':'团体缴费'}}<br><br>在另外两个选项卡中可以查看具体缴费方式说明，也可以更换缴费方式。</div>
